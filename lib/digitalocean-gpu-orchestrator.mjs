...
export function createDigitalOceanGpuOrchestrator({
  ...
  async function checkCapacity({ refresh = false } = {}) {
    ...
    const value = {
      ...
      available,
      requestable: developerCloudEntitlement,
      ...
      reason: listedCapacity
        ...
        : developerCloudEntitlement
          ? `AMD Developer Cloud access is configured, but live MI300X capacity is not confirmed by the DigitalOcean API. No GPU billing has started.`
  ...
  async function reapExpiredLeases({ excludeLeaseIds = [] } = {}) {
    ...
    for (const droplet of droplets) {
      if (excluded.has(String(droplet.id))) continue
  ...
  async function dropletRequest(selectedRegion = region, selectedSize = size) {
  ...
  async function startLease({ dryRun = false, signal } = {}) {
    ...
    const candidateRegions = [...new Set([capacity?.region, region, 'nyc2', 'tor1'].filter(Boolean))]
    const candidateSizes = [...new Set([size, size.replace(/-devcloud$/, '')].filter(Boolean))]
    let created = null
