            ...
            shots: {
              ...
              items: {
                ...
                properties: {
                  ...
                  shotRole: { type: 'string' },
                  lens: { type: 'string' },
                  depthPlan: { type: 'string' },
                  lightingTransition: { type: 'string' },
                  sceneDynamics: { type: 'string' },
                  composition: { type: 'string' },
                  stagecraft: { type: 'string' },
                },
              },
            },
          },
  ...
  return {
    ...
    videoDirection: {
      ...
      shots: Array.from({ length: 5 }, (_, index) => ({
        ...
        shotRole: index === 0
          ? 'opening silhouette reveal with the complete product readable by the end'
          : index === 4
            ? 'final hero proof frame for seller review'
            : 'visible-detail inspection beat grounded in source evidence',
        lens: index === 4
          ? '55mm final-packshot lens with stable framing and crisp visible marks'
          : '50mm commercial product lens with controlled depth and no product distortion',
        depthPlan: 'Use background depth separation and a gentle focus settle while the product ends sharp and complete.',
        lightingTransition: 'Move a soft highlight across the visible product surface, then settle into faithful commercial light.',
        sceneDynamics: 'Create visible motion through camera travel, light sweep, and background parallax only; the product itself does not morph.',
        composition: 'Keep the product large, complete, dominant, and reviewable with clean margins.',
        stagecraft: 'Use a minimal product-safe stage with no unverified props, overlays, extra products, or foreground occluders.',
...
function buildAgentPrompt(input) {
  ...
  return [
    ...
    'For each videoDirection shot, include cinematic director fields: shotRole, lens, depthPlan, lightingTransition, sceneDynamics, composition, and stagecraft. These must describe visible motion from start frame to end frame, depth, lighting, and scene treatment without changing or occluding the product.',
...
function buildProductStoryAgentPrompt(input) {
  ...
  return [
    ...
    'For a product group or set, keep every visible item fully inside the frame from start through end with a safety margin. Do not plan a detail shot that crops, isolates, or blurs away any member of the group.',
    sourceCount === 1
      ? 'Single-view evidence rule: do not request an Arc, orbit, rotation, profile reveal, or any camera angle that exposes an unseen side. Use front-plane Dolly In, Dolly Out, Dolly Left, Dolly Right, Crane Up, or Crane Down while keeping the visible face and silhouette intact.'
      : 'Multi-view evidence rule: use front-plane moves and do not request an Arc or orbit. A new side may appear as a cut to another source only when that exact side is directly supported by that source view.',
...
async function serveAmdWorkerFile(req, res, pathname) {
  const allowedFiles = new Set(['bootstrap.sh', 'app.py', 'gpu_telemetry.py', 'identity_guard.py', 'requirements.txt', 'run_story_pipeline.py', 'run_story_pipeline.sh'])
