...
function productLightingCue(value) {
  ...
  if (/low[- ]key|contrast|shadow/i.test(lighting)) return 'neutral luminance-only grazing light reveals texture without changing product hue'
...
function productStageCue(value) {
  ...
  const gradientPattern = /\bgradient\b/
  const floorPlanePattern = /\b(?:floor|ground)(?:\s+plane)?\b/
  ...
  const hasGradient = !cueIsNegated(environment, gradientPattern) && gradientPattern.test(environment)
  const hasFloorPlane = !cueIsNegated(environment, floorPlanePattern) && floorPlanePattern.test(environment)
  if (hasGradient && hasFloorPlane) return 'layered neutral gradient studio with a subtle floor plane'
  if (hasGradient) return 'layered neutral gradient studio'
  if (hasFloorPlane) return 'neutral studio with a subtle floor plane'
...
function productSceneCue(sceneDynamics, motionAffordances) {
  ...
  if (/reflection|specular|highlight/.test(direction)) {
    return 'Hue/saturation fixed; luminance varies; achromatic reflection moves behind product'
  }
...
export function compileProductRenderPrompt({
  ...
  const peopleClause = allowPeople
    ...
    : 'Product-only frame.'
  const prompt = [
    'Reference/color lock: preserve supplied product/set exactly—count, arrangement, geometry, exact source hue/saturation, material, marks, and text.',
    `Camera: ${cameraPreset}; ease-in, travel, ease-out, then hold the complete hero frame for 0.5s.`,
    ...
    preserveFullGroup
      ? 'Group lock: all items ≥6% edge-clear; preserve source spacing/overlap/clearance.'
      : '',
    peopleClause,
  ].filter(Boolean).join(' ')
  return {
