...
test('compiles a short reference-locked I2V motion prompt', () => {
  ...
  assert.match(result.prompt, /Reference\/color lock: preserve supplied product\/set exactly/)
  assert.match(result.prompt, /exact source hue\/saturation/)
  assert.match(result.prompt, /Camera: Crane Up; ease-in, travel, ease-out/)
