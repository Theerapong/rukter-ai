...
test('uses only evidence-supported directed shots without repeating a group-shot plan', () => {
  ...
  assert.ok(plan.shots.every((shot) => /preserve supplied product\/set exactly/i.test(shot.renderPrompt)))
  assert.ok(plan.shots.every((shot) => /Group framing: keep every item fully visible throughout/i.test(shot.renderPrompt)))
